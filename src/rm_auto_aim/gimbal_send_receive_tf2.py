...
class AutoAim_Data_Rx:
    def __init__(self, Pos_x, Pos_y, Pos_z, Vel_x, Vel_y, Vel_z,Yaw,V_yaw,Radius_1):
        self.Pos_x = Pos_x
        self.Pos_y = Pos_y
        self.Pos_z = Pos_z
        self.Vel_x = Vel_x
...
def pack_autoaim_data(data):
    return struct.pack('<fffffffff', data.Pos_x, data.Pos_y, data.Pos_z, data.Vel_x, data.Vel_y,  data.Vel_z, data.Yaw, data.V_yaw, data.Radius_1)
...
def build_autoaim_message(data):
    ...
    return struct.pack('<BBB', HEADER, length, CMD_ID_AUTOAIM_DATA_RX) + data_bytes
...
def send_auto_aim(Pos_x, Pos_y, Pos_z, Vel_x, Vel_y, Vel_z, Yaw, V_yaw, Radius_1):
    data = AutoAim_Data_Rx(Pos_x, Pos_y, Pos_z, Vel_x, Vel_y, Vel_z,Yaw, V_yaw, Radius_1)
...
class SPNode(Node):
    def __init__(self):
        ...
        self.publisher_timer = self.create_timer(0.0005,self.publish_message)
    def listener_callback(self, msg):
        # print(msg.position.x)
        send_auto_aim(msg.position.x, msg.position.y, msg.position.z, msg.velocity.x, msg.velocity.y, msg.velocity.z, msg.yaw, msg.v_yaw, msg.radius_1)
