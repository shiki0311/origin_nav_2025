...
class SPNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        q = tf_transformations.quaternion_from_euler(gimbal_msg.roll, gimbal_msg.pitch, gimbal_msg.yaw)
        t.transform.rotation.x = q[0]
        t.transform.rotation.y = q[1]
        t.transform.rotation.z = q[2]
        t.transform.rotation.w = q[3]
        self.br.sendTransform(t)
        self.publish_message(gimbal_msg)
    def publish_message(self, data):
        # msg = Gimbal
        # msg.data = data
        self.publish_gimbal.publish(data)
